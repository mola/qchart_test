...
// TableSeries = new TableModelSeries();
  ...
  SpinBoxDelegate = new SpinBoxThick(ui->SeriesTable);
  ...
  auto  item0 = new QStandardItem("1");

  item0->setToolTip("for test");
  item0->setIcon(QIcon("mainhub.png"));

  auto                    item1 = new QStandardItem("2");
  auto                    item2 = new QStandardItem("3");
  ...
  s->updateColor(Qt::red);
  s->updateSeriesLine(false);

  items << item0 << item1 << item2;

  TableSeries->insertRow(0, items);
